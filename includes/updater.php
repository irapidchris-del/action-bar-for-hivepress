 ...
 */
const UPDATE_REASON_KEY = 'action_bar_for_hivepress_release_reason';
 ...
 */
const UPDATE_RATE_LIMIT_KEY = 'action_bar_for_hivepress_release_rate_limit';
...
function bootstrap( $file ) {
	...
	// The update list itself, read side: see inject_update() for why core's own refresh is not
	// enough on its own.
	add_filter( 'site_transient_update_plugins', __NAMESPACE__ . '\\inject_update' );

	// The Plugins screen bulk action. One copy of this updater registers the action and the notice
	// (whichever loads first); every copy answers it for its own plugin.
	...
	if ( empty( $GLOBALS['hpx_update_check_bulk'] ) ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.
		$GLOBALS['hpx_update_check_bulk'] = __NAMESPACE__; // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- shared handshake between every copy of this updater; a plugin-specific prefix would defeat it.
...
function show_update_check_notice() {
	...
	if ( 'available' === $status ) {
		...
		$message = __( 'No releases have been published for Action Bar for HivePress yet, so there is nothing to update to. This is normal for a brand new copy and does not mean anything is wrong.', 'action-bar-for-hivepress' );
		$class   = 'notice-info';
	} elseif ( 'limited' === $status ) {
		$message = __( 'GitHub limits how many update checks one server may make each hour, and this server has reached that limit. Nothing is wrong with the plugin or your site, and checking will work again within the hour.', 'action-bar-for-hivepress' );
...
function inject_update( $transient ) {
	if ( ! is_object( $transient ) || ! plugin_file() ) {
		return $transient;
	}
...
function add_bulk_check( $actions ) {
	if ( current_user_can( 'update_plugins' ) ) {
		$actions['hpx_check_updates'] = __( 'Check for updates', 'action-bar-for-hivepress' );
	}
...
function show_bulk_check_notice() {
	...
	if ( $available ) {
		...
		$message = sprintf( _n( 'Checked %1$s plugin for updates: %2$s can be updated.', 'Checked %1$s plugins for updates: %2$s can be updated.', $checked, 'action-bar-for-hivepress' ), number_format_i18n( $checked ), number_format_i18n( $available ) );
	} else {
		/* translators: %s: number of plugins checked. */
		$message = sprintf( _n( 'Checked %s plugin for updates: it is up to date.', 'Checked %s plugins for updates: all are up to date.', $checked, 'action-bar-for-hivepress' ), number_format_i18n( $checked ) );
	}
